<%@page import="kr.co.kreamv.vo.BuyBidVO"%>
...
BuyBidVO buyBidVo = (BuyBidVO)request.getAttribute("buyBidVo");
ProdVO prodVo = (ProdVO)request.getAttribute("prodVo");
SellBidVO sellBidVo = null;
int type = (int)request.getAttribute("type");
if(type==3){
	sellBidVo = (SellBidVO)request.getAttribute("sellBidVo");
}
%>
...
$(function(){
	...
	<%if(buyBidVo==null){%>	// 즉시 판매가가 없을 경우 즉시구매 버튼 비활성화
	$('#orderBtn').attr('disabled', 'disabled');
<%}%>

	$('#orderBtn').on('click', function(){	// 판매입찰 버튼 클릭 -> 판매입찰 form 출력, 즉시판매 form 숨기기
		$('#bid').hide();
		$('#orders').show();
	})
	$('#bidBtn').on('click', function(){	// 즉시판매 버튼 클릭 -> 판매입찰 form 숨기기, 즉시판매 form 출력
		$('#bid').show();
		$('#orders').hide();
	})
})
...
<div class="container2"> 
   ...
   <hr>
	...
	<input class="button btn btn-dark" id="bidBtn" type="submit" value="판매 입찰">	
	<input class="button btn btn-dark" id="orderBtn" type="submit" value="즉시 판매">	<br>
	
	<% 
      String amount = String.valueOf(buyBidVo.getBuy_bid_price());
      amount = amount.replaceAll("\\B(?=(\\d{3})+(?!\\d))", ",");
     %>

	<hr>
	<%if(buyBidVo!=null) {%>
	<form id="orders" action="<%=request.getContextPath()%>/sellOrdersTemp.do">
	<a><b>즉시 판매가</b></a>
	<a><b><%=amount%>원</b></a>
	<hr>
		
	<div>
	<input type="text" class="detailIdBtn" name="confirm_id" value="<%=request.getAttribute("confirm_id")%>">
	...
	<input type="text" class="detailIdBtn" name="buy_bid_id" value="<%=buyBidVo.getBuy_bid_id()%>">	
	<input type="text" class="detailIdBtn" name="buy_bid_price" value="<%=buyBidVo.getBuy_bid_price()%>">
	<input type="text" class="detailIdBtn" name="storeType" value="<%=request.getAttribute("type")%>">
	<input class="button3 btn btn-dark" id="buyGo" type="submit" value="즉시 판매 계속">
	</div>
	<hr>
	</form>
	<%} %>
...
<form id="bid" action="<%=request.getContextPath() %>/sellBidTemp.do" method="get">
<a><b>판매 희망가</b></a>
	<input type="text" class="detailIdBtn" name="confirm_id" value="<%=request.getAttribute("confirm_id")%>">
	<input type="text" class="detailIdBtn" name="datail_id" value="<%=prodDetailVo.getDetail_id()%>">
	<input type="text" class="detailIdBtn" name="prod_id" value="<%=prodVo.getProd_id()%>">
	<input type="text" class="detailIdBtn" name="storeType" value="<%=request.getAttribute("type")%>">
	<%if(type==2){ %>	<!-- 입찰하지 않은 보관상품일 경우 -->
	<input type="text"  name="sell_bid_price" placeholder="희망가입력" style="text-align:right;">원
	<%}else{ %>			<!-- 입찰했던 보관상품일 경우 기존의 입찰가격 보여주기 -->
	<input type="text"  name="sell_bid_price" placeholder="<%=sellBidVo.getSell_bid_price()%>">원
	<%} %>
	<div>
	<hr>
	<a><b>입찰 마감 기한</b></a>
	<input id="deadLine" type="text" name="sell_bid_fdate">
	<br>
	...
	<div>
	<input class="button3 btn btn-dark" type="submit" value="판매 입찰 계속">
	</div>
