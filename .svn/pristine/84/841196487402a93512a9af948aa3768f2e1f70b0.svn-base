	...
	$.ajax({
		...
		success : function(res){
			...
			$.each(res,function(i,v){
				...
				code += '			 		<a class="action" name="list" data-toggle="collapse" data-parent="#accordion" href="#collapse' + v.board_num + '" onclick="boardTitle()">'
				...
				code += '	 		<div id="collapse' + v.board_num + '" class="panel-collapse collapse">                                                        '
					...
					code +=	'				</p>                                                                                                                  '
					code +=	'				                                                                                                                      '
					code +=	'	 			<p class="p2">' + v.board_cont + '</p>'
					code += 			   '<input type = "button" class = "acTion' + v.mem_id + '" value = "수정" style = "display : none">'
					code += 			   '<input type = "button" class = "acTion' + v.mem_id + '" value = "삭제" style = "display : none">'
					code +=				'</div>'
			    code +=				'</div>'
				code +=			'</div>'
			}) //each   
...
<%-- // 게시글 제목을 눌렀을때 관리자면
$(document).on("click", ".title", function(){
		var uidAdmin = '<%=session.getAttribute("loginMember")%>';
  	  	 memId = $(this).attr('id');
                ...
                <div class="snb_list">
                    <ul class="snb_menu">
                        <li class="menu_item menu_on" >
                        	<a href="/kreamv/view/askBoard.jsp" class="menu_link nuxt-link-exact-active nuxt-link-active" aria-current="page">배송 문의</a>
                        </li>
                        <li class="menu_item" >
                        	<a href="/kreamv/view/askBoard1.jsp" class="menu_link">상품 문의</a>
                        </li>
                    </ul>
                </div>
            </nav>
        ...
        <div class="customer_content my_notice">
            ...
            <!-- 게시글 목록 -->
				...
				<button type="button" class="btn btn-dark" id="writebtn"
					data-toggle="modal" data-target="#wModal">글쓰기</button>
					...
					<div class="modal-dialog">
						<div class="modal-content">
							...
							<div class="modal-body">
								<form id="wform">
									<input type="hidden" id="wloginId"
										value="<%=session.getAttribute("loginId") %>"> <label>ID
										:</label>
									<p><%=session.getAttribute("loginId") %></p>
									<br> <label>제목 </label> <input type="text" id="title"><br>

									<label>내용 </label><br>
									<textarea rows="10" id="cont" cols="50"></textarea>
									<br> <input type="submit" value="저장하기" id="wsend">
								</form>
							</div>

							<!-- Modal footer -->
							<div class="modal-footer">
								<button type="button" id="modalCloseBtn" class="btn btn-dark"
